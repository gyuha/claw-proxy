...
The two biggest planning risks are Anthropic protocol coverage and Claude Code gateway compatibility. Anthropic’s current API requires `x-api-key`, `anthropic-version`, and JSON requests to `POST /v1/messages`, while Claude Code’s gateway contract additionally expects `/v1/messages/count_tokens` and forwarding of `anthropic-beta` and `anthropic-version`. [CITED: https://platform.claude.com/docs/en/api/overview] [CITED: https://platform.claude.com/docs/en/api/versioning] [CITED: https://code.claude.com/docs/en/llm-gateway] The scaffold does not expose `count_tokens`, does not model headers, and currently collapses both OpenAI and Anthropic message content into `String`. [VERIFIED: core/src/proxy/mod.rs; core/src/normalizer/mod.rs; core/src/normalizer/from_openai.rs; core/src/normalizer/from_anthropic.rs]

**Primary recommendation:** Plan Phase 1 as a strict text-only, non-streaming MVP that preserves the current dual-route architecture, adds the missing Claude Code gateway surfaces (`/v1/messages/count_tokens` and Anthropic header handling), makes the OpenAI adapter injectable for tests, and explicitly rejects unsupported multipart/tool payloads with 4xx errors instead of silently coercing them. [VERIFIED: core/src/proxy/mod.rs; core/src/providers/openai.rs; core/src/normalizer/from_openai.rs; core/src/normalizer/from_anthropic.rs] [CITED: https://code.claude.com/docs/en/llm-gateway] [CITED: https://developers.openai.com/api/reference/resources/chat/subresources/completions/methods/create]
...
| Claude Code support | `ANTHROPIC_BASE_URL` is the documented gateway entrypoint. [CITED: https://code.claude.com/docs/en/llm-gateway] | The gateway contract is not met until `/v1/messages/count_tokens` exists and required headers are preserved. [VERIFIED: core/src/proxy/mod.rs] [CITED: https://code.claude.com/docs/en/llm-gateway] |
...
| Claude Code integration | Custom client patching or local Claude Code forks [ASSUMED] | Documented gateway environment variables such as `ANTHROPIC_BASE_URL`, `ANTHROPIC_MODEL`, and `ANTHROPIC_DEFAULT_*`. [CITED: https://code.claude.com/docs/en/llm-gateway] [CITED: https://code.claude.com/docs/en/model-config] | Phase 1 should adapt the proxy to the documented client contract, not the other way around. [VERIFIED: .planning/REQUIREMENTS.md] |
...
|--------------|------------------|--------------|--------|
| Anthropic gateway compatibility assumed only `POST /v1/messages` [ASSUMED] | Claude Code gateway docs now require Anthropic Messages format with `/v1/messages`, `/v1/messages/count_tokens`, and header forwarding for `anthropic-beta` plus `anthropic-version`. [CITED: https://code.claude.com/docs/en/llm-gateway] | Verified against current docs on 2026-04-10. [VERIFIED: research session] | Phase 1 planning must include `count_tokens` and header preservation if PROXY-04 is meant literally. [VERIFIED: .planning/REQUIREMENTS.md] |
...
| A2 | Claude Code model pinning env vars can be used to point aliases such as Sonnet at proxy-accepted model IDs, avoiding server-side alias mapping in MVP. [ASSUMED] | Open Questions; Common Pitfalls | If Claude Code or users do not accept that configuration burden, Phase 1 will need server-side model alias translation. |
...
1. **What is the minimum Anthropic feature subset required for “normal Claude workflow”?** [VERIFIED: .planning/REQUIREMENTS.md]  
   What we know: Claude Code gateway compatibility requires `/v1/messages`, `/v1/messages/count_tokens`, and forwarding of `anthropic-beta` plus `anthropic-version`. [CITED: https://code.claude.com/docs/en/llm-gateway]  
   What’s unclear: Whether tool-use blocks are required on day one for the intended local workflow. [ASSUMED]  
   Recommendation: Capture one real Claude Code request trace against a compliant gateway before writing the Phase 1 plan, or explicitly narrow PROXY-04 to “text-only Anthropic workflows.” [ASSUMED]

2. **How should Anthropic-facing model names map onto the OpenAI adapter in MVP?** [VERIFIED: core/src/providers/openai.rs; .planning/ROADMAP.md]  
   What we know: Claude Code supports `ANTHROPIC_MODEL` and `ANTHROPIC_DEFAULT_*` environment variables for model selection and pinning. [CITED: https://code.claude.com/docs/en/model-config]  
   What’s unclear: Whether the product should require client-side model pinning or provide server-side alias mapping for a smoother UX. [ASSUMED]  
   Recommendation: Lock this decision before Plan `01-02`; it changes both config shape and acceptance tests. [ASSUMED]

3. **Should Phase 1 support `stream: true` at all?** [VERIFIED: core/src/normalizer/mod.rs; core/src/providers/openai.rs]  
   What we know: Both the internal model and caller schemas already carry a `stream` flag, but the OpenAI adapter forces `stream: false`. [VERIFIED: core/src/normalizer/mod.rs; core/src/providers/openai.rs]  
   What’s unclear: Whether any Phase 1 caller is expected to use streaming before Phase 2 observability work. [ASSUMED]  
   Recommendation: Treat streaming as out of scope for Phase 1 unless the planner explicitly adds it, and reject it clearly rather than ignoring it. [ASSUMED]
...
| PROXY-04 | Claude Code gateway surfaces exist and preserve required Anthropic headers/count-token path. [VERIFIED: .planning/REQUIREMENTS.md] [CITED: https://code.claude.com/docs/en/llm-gateway] | integration + one manual smoke | `cargo test -p claw-proxy-core anthropic_gateway_contract -- --exact` [ASSUMED] | ❌ Wave 0 |
...
- **Phase gate:** Full suite green plus one manual `curl` smoke for both public proxy routes and one manual Claude Code gateway smoke if PROXY-04 remains in scope literally. [VERIFIED: .planning/ROADMAP.md; .planning/REQUIREMENTS.md] [ASSUMED]
