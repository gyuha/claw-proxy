...
- Tauri 메뉴바(시스템 트레이) 앱 — 아이콘 클릭 시 사이드바 팝오버 창 표시
- 코어 엔진과 UI 완전 분리
...
### 8.1 앱 형태 — 메뉴바(시스템 트레이) 앱

- macOS: 상단 메뉴바에 아이콘 상주 (독(Dock)에 표시 안 함)
- Windows: 시스템 트레이에 아이콘 상주
- 아이콘 클릭 시 **사이드바 팝오버 창** 표시 (아이콘 바로 아래 팝업)
- 창은 포커스를 잃으면 자동으로 숨김 (dismiss on blur)
- Tauri 2.x `tray-icon` 플러그인 + `tauri-plugin-positioner` 로 위치 고정
...
- Tauri 2.x
  - `tauri-plugin-shell` — 코어 사이드카 프로세스 관리
  - `tauri-plugin-positioner` — 트레이 팝오버 창 위치 고정
  - `tray-icon` (Tauri 내장) — 메뉴바/시스템 트레이 아이콘
- React 18
- TypeScript
- TailwindCSS
- shadcn/ui
